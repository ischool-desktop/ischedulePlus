...
namespace ischedulePlus
{
    ...
    public partial class ClassCreatorForm : FISCA.Presentation.Controls.BaseForm
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!string.IsNullOrEmpty(errorGradeYear.GetError(txtGradeYear)) || string.IsNullOrEmpty(txtGradeYear.Text))
                {
                    MsgBox.Show("年級必須為數字！");
                    return;
                }
                ...
                try
                {
                    mDataAccess.Insert_new(NewName + "," + txtGradeYear.Text, "" + cmbNames.SelectedItem, textBoxX1.Text);
                }
                catch (Exception ex)
                {
